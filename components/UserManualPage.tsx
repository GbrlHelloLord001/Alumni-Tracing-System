...
import { Download, FileText, CheckCircle, Info, LayoutTemplate, Layers, Inbox, Bot, Globe, Database, Users, Shield, BookOpen, UserCheck, Briefcase, ZoomIn, ZoomOut, Maximize } from 'lucide-react';
import html2canvas from 'html2canvas';
import jsPDF from 'jspdf';

const UserManualPage: React.FC = () => {
    const [isDownloading, setIsDownloading] = useState(false);
    const [viewScale, setViewScale] = useState(0.75); // Default scaled down for easier viewing

    const downloadPDF = async () => {
        setIsDownloading(true);
        const originalScale = viewScale;
        
        // Temporarily reset scale to 100% for high-quality, un-warped capture
        setViewScale(1);
        
        // Wait for React to re-render the unscaled version
        ...
        try {
            ...
            setViewScale(originalScale); // Restore user's preferred scale
            setIsDownloading(false);
        }
    };
    ...
    return (
        <div className="max-w-7xl mx-auto py-8 font-sans">
            <div className="flex justify-between items-end mb-8 pl-4 pr-1">
                <div>
                ...
                <div className="flex items-center gap-4">
                    {/* Zoom Controls */}
                    <div className="hidden md:flex items-center gap-1 bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm">
                        <button 
                            onClick={() => setViewScale(Math.max(0.4, viewScale - 0.1))}
                        ...
                        <button 
                            onClick={() => setViewScale(Math.min(1.5, viewScale + 0.1))}
                            className="p-2 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                            title="Zoom In"
                        >
                            <ZoomIn size={18} />
                        ...
                        <button 
                            ...
                            className={`p-2 rounded-lg transition-colors text-sm font-bold ${viewScale === 1 ? 'bg-slate-100 text-slate-800' : 'text-slate-500 hover:text-blue-600 hover:bg-blue-50'}`}
                            title="Actual Size"
                        >
                            <Maximize size={16} />
                        </button>
            ...
            <div className="bg-slate-200/50 rounded-3xl p-8 overflow-x-auto custom-scrollbar flex flex-col items-center h-[80vh] overflow-y-auto border border-slate-200 shadow-inner">
                
                {/* Scaled Wrapper */}
                <div 
                    className="transition-all duration-300 ease-in-out"
                    style={{ 
                        width: `${1123 * viewScale}px`, 
                        height: `${5750 * viewScale}px` 
                    }}
                >
                    <div 
                        className="flex flex-col items-center gap-8 origin-top-left transition-transform duration-300 ease-in-out"
                        style={{ transform: `scale(${viewScale})`, width: '1123px' }}
                    >
                
                {/* PAGE 1: Cover Page */}
                <div className="pdf-page flex-shrink-0 w-[1123px] h-[794px] bg-white shadow-xl relative overflow-hidden p-12 flex flex-col justify-center items-center text-center border-8 border-indigo-50" style={{ boxSizing: 'border-box' }}>
                    ...
                    <div className="relative z-10 w-full flex flex-col items-center">
                        <div className="w-28 h-28 bg-blue-600 rounded-[2rem] flex items-center justify-center text-white shadow-2xl shadow-blue-500/30 mb-8 transform -rotate-3 hover:rotate-0 transition-transform">
                            <Layers size={56} />
                        </div>
                        <h1 className="text-[4rem] leading-[1.1] font-black text-slate-800 tracking-tight mb-4">
                            Alumni <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Tracer</span> System
                        </h1>
                        <h2 className="text-2xl font-bold text-slate-400 uppercase tracking-widest mb-12">User Manual & Configuration Guide</h2>
                        
                        <div className="w-full max-w-2xl bg-white p-8 rounded-[2rem] shadow-2xl shadow-slate-200/50 border border-slate-100 flex flex-col gap-5 text-left">
                ...
                <div className="pdf-page flex-shrink-0 w-[1123px] h-[794px] bg-white shadow-xl relative overflow-hidden p-12 flex flex-col justify-between" style={{ boxSizing: 'border-box' }}>
                    
                    <div className="flex justify-between items-center border-b-2 border-slate-100 pb-6 mb-6">
                    ...
                    <div className="flex flex-row gap-10 flex-grow">
                        <div className="w-1/3 bg-slate-50/80 rounded-[2rem] p-8 border border-slate-100">
                            <h4 className="text-xl font-black text-slate-800 mb-6 flex items-center gap-3 uppercase tracking-wider">
                                Table of Contents
                            </h4>
                            <ul className="space-y-4 font-bold text-slate-600">
                                <li>
                                    <div className="flex items-center gap-3 text-blue-600">
                                        <span className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-xs">1</span> Introduction & Overview
                                    </div>
                                </li>
                                <li>
                                    <div className="flex items-center gap-3">
                                        <span className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-xs">2</span> Alumni Portal Features
                                    </div>
                                    <ul className="mt-3 ml-9 space-y-2 text-slate-500 font-medium list-disc list-inside text-sm">
                                ...
                                <li className="mt-4">
                                    <div className="flex items-center gap-3">
                                        <span className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center text-xs">3</span> Admin Master Setup
                                    </div>
                                    <ul className="mt-3 ml-9 space-y-2 text-slate-500 font-medium list-disc list-inside text-sm">
                        ...
                        <div className="w-2/3 pr-8 flex flex-col justify-center">
                            <h4 className="text-2xl font-black text-slate-800 mb-3">Introduction to the System</h4>
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                <div className="bg-white p-4 rounded-2xl border-2 border-blue-100 shadow-sm flex items-start gap-4">
                                    <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center flex-shrink-0 mt-1">
                                        <UserCheck size={20} />
                                    </div>
                                    <div>
                                        <h5 className="text-lg font-bold text-slate-800 mb-1">The Alumni Perspective</h5>
                                ...
                                <div className="bg-white p-4 rounded-2xl border-2 border-indigo-100 shadow-sm flex items-start gap-4">
                                    <div className="w-10 h-10 bg-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center flex-shrink-0 mt-1">
                                        <LayoutTemplate size={20} />
                                    </div>
                                    <div>
                                        <h5 className="text-lg font-bold text-slate-800 mb-1">The Administrative Capacity</h5>
                                        <p className="text-slate-600 leading-relaxed text-xs">
                ...
                <div className="pdf-page flex-shrink-0 w-[1123px] h-[794px] bg-white shadow-xl relative overflow-hidden p-12 flex flex-col justify-between" style={{ boxSizing: 'border-box' }}>
                    ...
                    <div className="grid grid-cols-2 gap-10 flex-grow">
                        <div className="flex flex-col">
                            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-sky-400 to-blue-600 text-white flex items-center justify-center shadow-lg shadow-blue-200 mb-5">
                                <Shield size={28} />
                            </div>
                            <h4 className="text-2xl font-bold text-slate-800 mb-3">Registration & Verification</h4>
                            ...
                            <ul className="space-y-3 text-slate-700 bg-slate-50 p-5 rounded-2xl border border-slate-100 text-sm">
                        ...
                        <div className="flex flex-col">
                            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-400 to-purple-600 text-white flex items-center justify-center shadow-lg shadow-indigo-200 mb-5">
                                <LayoutTemplate size={28} />
                            </div>
                            <h4 className="text-2xl font-bold text-slate-800 mb-3">Profile Dashboard & State Management</h4>
                            <p className="text-slate-600 leading-relaxed mb-5 text-sm">
                            ...
                            <div className="mt-6 bg-amber-50 p-4 rounded-xl border border-amber-200 flex gap-4 items-start">
                ...
                <div className="pdf-page flex-shrink-0 w-[1123px] h-[794px] bg-white shadow-xl relative overflow-hidden p-12 flex flex-col justify-between" style={{ boxSizing: 'border-box' }}>
                    ...
                    <div className="grid grid-cols-2 gap-10 flex-grow">
                        <div className="flex flex-col">
                            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-400 to-teal-600 text-white flex items-center justify-center shadow-lg shadow-emerald-200 mb-5">
                                <UserCheck size={28} />
                            </div>
                            <h4 className="text-2xl font-bold text-slate-800 mb-3">Digital Alumni ID Generation</h4>
                            <p className="text-slate-600 leading-relaxed mb-5 text-sm">
                                Serves as a definitive, verifiable token of university affiliation. Rendered natively in the browser and fully exportable.
                            </p>
                            <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 flex-grow flex flex-col justify-center items-center">
                                <div className="border-4 border-indigo-100 rounded-xl p-4 bg-white shadow-xl mb-4 relative hover:-translate-y-2 transition-transform duration-300 w-full max-w-[280px]">
                                    <div className="flex items-center gap-4 mb-3 border-b border-slate-100 pb-3">
                                        <div className="w-10 h-10 rounded-full bg-slate-200"></div>
                                        <div>
                                            <div className="w-20 h-2.5 bg-slate-200 rounded mb-2"></div>
                                ...
                                <p className="text-center text-xs font-bold text-slate-500">Automated High-Resolution Canvas Export</p>
                            </div>
                        </div>

                        <div className="flex flex-col">
                            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-rose-400 to-orange-600 text-white flex items-center justify-center shadow-lg shadow-rose-200 mb-5">
                                <Briefcase size={28} />
                            </div>
                            <h4 className="text-2xl font-bold text-slate-800 mb-3">University Job Board</h4>
                            <p className="text-slate-600 leading-relaxed mb-5 text-sm">
                                An exclusive employment portal where alumni browse verified active postings curated from reputable university partners.
                            </p>
                            <ul className="space-y-4 text-slate-700 bg-slate-50 p-5 rounded-2xl border border-slate-100">
                                <li>
                ...
                <div className="pdf-page flex-shrink-0 w-[1123px] h-[794px] bg-white shadow-xl relative overflow-hidden p-12 flex flex-col justify-between" style={{ boxSizing: 'border-box' }}>
                    ...
                    <div className="grid grid-cols-2 gap-8 flex-grow">
                        <div className="flex flex-col gap-5">
                            <div className="bg-slate-50 rounded-3xl p-5 border border-slate-100">
                                <div className="flex items-center gap-4 mb-2">
                                    ...
                                    <h4 className="text-lg font-bold text-slate-800">Master Analytics</h4>
                                </div>
                            ...
                            <div className="bg-slate-50 rounded-3xl p-5 border border-slate-100 flex-grow">
                                <div className="flex items-center gap-4 mb-2">
                                    <div className="w-10 h-10 bg-rose-100 text-rose-600 rounded-2xl flex items-center justify-center">
                                        <Inbox size={20} />
                                    </div>
                                    <h4 className="text-lg font-bold text-slate-800">Updates Inbox</h4>
                                </div>
                                ...
                                <div className="p-3 bg-white rounded-xl border border-slate-200">
                                    <p className="text-xs font-bold text-slate-800 mb-2">Review Process Check:</p>
                        ...
                        <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-[2rem] p-6 text-white flex flex-col justify-center border border-slate-700 shadow-2xl relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-64 h-64 bg-fuchsia-500/20 rounded-full blur-3xl"></div>
                            
                            <div className="relative z-10 flex flex-col h-full justify-center">
                                <div className="w-14 h-14 bg-fuchsia-500/20 text-fuchsia-400 rounded-2xl flex items-center justify-center backdrop-blur-sm border border-fuchsia-500/30 mb-5">
                                    <Bot size={28} />
                                </div>
                                <h4 className="text-2xl font-black mb-2 tracking-tight">P.A.U.L.O. Assistant</h4>
                                <h5 className="text-sm font-medium text-fuchsia-300 mb-4">Predictive AI Utility & Logic Operator</h5>
                                
                                <p className="text-slate-300 leading-relaxed max-w-sm mb-5 text-sm">
                                ...
                                <div className="space-y-3">
                                    <div className="bg-white/10 p-3 rounded-xl backdrop-blur-md border border-white/10">
                                        <code className="text-xs text-fuchsia-200 font-mono">"Summarize the highest employment industry for BSIT."</code>
                                    </div>
                                    <div className="bg-white/10 p-3 rounded-xl backdrop-blur-md border border-white/10">
                                        <code className="text-xs text-fuchsia-200 font-mono">"Draft a professional email reminder for inactive users."</code>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-8 flex-grow">
                        ...
                        <div className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 shadow-sm flex flex-col">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h4 className="text-xl font-bold text-slate-800">User Database</h4>
                            </div>
                            <p className="text-slate-600 text-sm leading-relaxed mb-4">
                            ...
                            <div className="mt-auto flex items-center gap-2 text-indigo-600 font-bold bg-indigo-50 p-3 text-sm rounded-xl w-max">
                                <CheckCircle size={16} /> Features strict filtering by Array constraints
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 shadow-sm flex flex-col">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h4 className="text-xl font-bold text-slate-800">Alumni Fetcher</h4>
                            </div>
                            <p className="text-slate-600 text-sm leading-relaxed mb-4">
                        ...
                        <div className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 shadow-sm flex flex-col">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h4 className="text-xl font-bold text-slate-800">Reports Module</h4>
                            </div>
                            <p className="text-slate-600 text-sm leading-relaxed mb-4">
                        ...
                        <div className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 shadow-sm flex flex-col">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h4 className="text-xl font-bold text-slate-800">Graduate Bulk Import</h4>
                            </div>
                            <p className="text-slate-600 text-sm leading-relaxed mb-4">
                ...
                <div className="pdf-page flex-shrink-0 w-[1123px] h-[794px] bg-slate-800 text-white shadow-xl relative overflow-hidden p-12 flex flex-col justify-center items-center text-center border-8 border-slate-900" style={{ boxSizing: 'border-box' }}>
                    ...
                    <div className="relative z-10 w-full max-w-4xl flex flex-col items-center">
                        <div className="w-20 h-20 bg-slate-700 rounded-[2rem] flex items-center justify-center text-slate-300 border border-slate-600 mb-6">
                            <Shield size={40} />
                        </div>
                        <h4 className="text-4xl font-black text-white mb-5">Security & Architecture</h4>
                        ...
                        <div className="grid grid-cols-3 gap-5 w-full text-left">
                            <div className="bg-slate-700/50 p-6 rounded-2xl border border-slate-600">
                                <h5 className="text-base font-bold text-white mb-2">Row Level Security</h5>
                            ...
                            <div className="bg-slate-700/50 p-6 rounded-2xl border border-slate-600">
                                <h5 className="text-base font-bold text-white mb-2">Rate Limit Strategy</h5>
                                <p className="text-sm text-slate-300 leading-relaxed">
                            ...
                            <div className="bg-slate-700/50 p-6 rounded-2xl border border-slate-600">
                                <h5 className="text-base font-bold text-white mb-2">Encryption Vectors</h5>
                                <p className="text-sm text-slate-300 leading-relaxed">
                        ...
                        <div className="mt-12 text-slate-500 font-bold uppercase tracking-widest text-sm">
